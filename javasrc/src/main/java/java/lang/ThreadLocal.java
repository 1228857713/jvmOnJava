...
public class ThreadLocal<T> {
    ...
    static class ThreadLocalMap {
        ...
        /**
         * 设置扩容阙值,因为这个方法被私有化了,只能本类调用所以只有在设置构造方法的时候设置其大小
         * 其扩容阙值是 INITIAL_CAPACITY 的 2/3
         */
        ...
        private void set(ThreadLocal<?> key, Object value) {
            Entry[] tab = table;
            int len = tab.length;
            // 计算 key 在数组中的下标,这里直接使用thread的hashcode 与15(1111)取余快速得到其在数组中的下标
            int i = key.threadLocalHashCode & (len - 1);
            // 遍历一段连续的元素，以查找匹配的 ThreadLocal 对象
            // 1.这里使用的是线性地址探测法，方法也比较简单，如果当前的位置有对象那么直接找下一个位置，如果没有就用这一个位置
            // 2.线性地址探测法主要用于存放值比较少，hash 冲突不多的数据。不然hash 冲突比较严重会导致性能严重下降
