 ...
 * 总结：
 *     1.默认初始化容量为 16，负载因子为 0.75 也就是说当 map 的size 数量大于 12 的时候就会开始扩容
 *     2.当 table[i] 的size 大于8的时候就会 发生树化，前提也是当只有键值对数量大于 64 才会发生转换。
 *     3.树转链表的阀值，小于等于6是转为链表，仅在扩容tranfer时才可能树转链表
 *
 * @param <K>
 * @param <V>
 */
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    static class Node<K, V> implements Map.Entry<K, V> {
        ...
        volatile V val;
        //如果他有next 节点那么他就是列表，如果没有的话那么他就是数组的一个节点
        ...
        /**
         * HashMap中Node类的hashCode()方法中的代码为：Objects.hashCode(key) ^ Objects.hashCode(value)  (^ 按位异或运算)
    ...
    /**
     * 通过 cas 的方式获得在i位置上的Node节点位置保证每次得到的都是最新的
     */
    ...
    /**
     * 存放node的数组,大小是2的幂次方
     * ConcurrentHashMap 的数组是在put 第一个 数据时候才开始初始化的。这样会有很多线程并发的问题。
     ...
     *    2.2）说明table[i]的节点的hash值不等于MOVED，如果table[i]为链表节点，则将此节点插入链表中即可(尾插法)
     *          如果table[i]为树节点，则将此节点插入树中即可。插入成功后，进行 5
     * 5、如果table[i]的节点是链表节点，则检查table的第i个位置的链表是否需要转化为树，如果需要则调用treeifyBin函数进行转化
     */
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        // key和value不允许null,这点是其和 hashmap的区别，hashmap 的key是可以为空的
        ...
        for (Node<K, V>[] tab = table; ; ) {//CAS经典写法，不成功无限重试
             ...
             */
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {// 这里不管节点 是否等于赋值都会成功，所以f会变成列表的首节点
                if (casTabAt(tab, i, null, new Node<K, V>(hash, key, value, null))){
                    // 通过cas 操作将新的 node 节点放置到 对应的数组位置上，如果成功了代表 put 操作成功，跳出循环
                    ...
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) {
                            ...
                            for (Node<K, V> e = f; ; ++binCount) {
                                ...
                                Node<K, V> pred = e;
                                if ((e = e.next) == null) {//插入到链表末尾并跳出循环。所以ConcurrentHashMap 用的是尾插法
    ...
    /**
     *   put(key,value) 如果存在 会直接覆盖
     *   put(key,value,ifabsent)  如果存在不会覆盖
            ...
            if ((sc = sizeCtl) < 0)
                ...
                try {
                    ...
                    if ((tab = table) == null || tab.length == 0) {
                        ...
                        //例如16经过运算为12，与乘0.75一样，只不过位运算更快(这代码写的也太骚了)
                        sc = n - (n >>> 2);
                    }
                } finally {
    ...
    private final void transfer(Node<K, V>[] tab, Node<K, V>[] nextTab) {
        ...
        //初始化迁移后的新Node数组
        // 这里没有加锁，因为其他线程在添加的时候如果发现当前的 table在扩容就会帮组其扩容
