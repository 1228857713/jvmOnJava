...
/**
 * HashMap是常用的Java集合之一，
 *  1.是基于哈希表的Map接口的实现。与HashTable主要区别为不支持同步和允许null作为key和value。
 ...
 *  5.而JDK1.8中，HashMap采用数组+链表+红黑树实现，当链表长度超过阈值8时，将链表转换为红黑树，这样大大减少了查找时间。
 *      原本Map.Entry接口的实现类Entry改名为了Node。转化为红黑树时改用另一种实现TreeNode。
 *
 *  面试相关：
 *    1.HashMap 的初始化大小为 16，扩容因子为0.75。
 *      注意这个容量是 table的容量，即数组的容量，而正在的存放的数据的大小应该是数组的容量+列表的容量
 *    2.hashmap 的容量大小必定为2的整数次幂，这样在对这个容量取余的时候，可以直接通过且操作即可
 *    2.当列表的长度大于8的时候，列表会树化为红黑树
 *    3.当列表长度小于6的时候，红黑树会退化为列表。
 */
